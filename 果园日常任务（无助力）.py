...
class LYB:
    ...
    def share(self, actid1, shareId1):
        # 设置一个布尔标志来控制是否执行助力
        execute_assist = False  # 将其设置为False，禁用助力任务

        if not execute_assist:
            #print(f"[{self.name1}] 助力任务已被禁用")
            return None
...
if __name__ == '__main__':
    ...
    print(f"==========果园获取到 {len(zlck_list)} 个运行任务账号==========")
        ...
        if actid is None or shareId is None:
            ...
            for i, ck in enumerate(cookies):
                ...
                if zlcs < 3:
                    ...
                    continue
        print(f"======账号{dzl_num}果园任务-领取奖励并浇水======")
        lyb.main()
        print(f"======账号{dzl_num}-果园任务结束======\n\n")
